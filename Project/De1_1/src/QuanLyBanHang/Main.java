...
public class Main extends javax.swing.JFrame {
    ...
    public void docKhachHang(){
        String s="KhachHang.txt";
        File f=new File(s);
        if(f.exists()){
            IOFile.readFile(kh, s);
            dm1.setRowCount(0);
            for(KhachHang as:kh){
                    Vector v=new Vector();
    ...
    public void docMatHang(){
        ...
        File f=new File(s);
        if(f.exists()){
            IOFile.readFile(mh, s);
            dm2.setRowCount(0);
            for(MatHang as:mh){
                Vector v=new Vector();
                v.add(String.valueOf(as.getMa()));
                v.add(as.getTenMa());
                v.add(as.getNhomMa());
                v.add(String.valueOf(as.getNhomMa()));
                a4.add(v);
            }tb2.updateUI();
        }
    }
    ...
    private void btnThemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnThemActionPerformed
        ...
        String ss="KhachHang.txt";
            try {
                    IOFile.writeFile(kh, ss);
            } catch (Exception e) {
